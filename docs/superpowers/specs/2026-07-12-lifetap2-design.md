...
- A **commander-damage grid** pinned to the player-facing bottom edge (seat-rotated with the zone), above the `Listener`: a compact **two-column grid** of small rounded cells (radius 8, dark `#1E1E1E` surface with a thin `#2C2C2C` border), one per **opponent**, each holding a small shield icon (`Icons.shield`) tinted in that opponent's color and the damage this player has taken from them (default 0). Tapping a cell = **+1** commander damage from that opponent (dispatches `AdjustCommanderDamage` with `reduceLife` set from the "Commander damage life loss" setting); long-press = **−1**, clamped at 0 (disabled at 0). A cell at **21+** — lethal — flags red, and with life-loss on the KO treatment then applies. Each cell carries `HitTestBehavior.opaque` and keeps its `ValueKey('cmdr-<playerId>-<oppId>')`, so only its own hit area is consumed; the rest of the cell still taps life. The poison/energy/experience counters stay a separate compact chip cluster just above the grid.
